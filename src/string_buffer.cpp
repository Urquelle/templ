...
sb_push(String_Buffer *sb, char *str) {
    String_Buffer *result = sb;

    if ( !result ) {
        result = ALLOC_STRUCT(&sys_arena, String_Buffer);
        result->cap = STRING_BUFFER_CAP;
        result->len = 0;
        result->size = 0;
        result->buf = (char *)ALLOC_SIZE(&sys_arena, STRING_BUFFER_CAP);
    }
    ...
    if ( result->cap < (result->size + size) ) {
        ...
        char *buf = (char *)ALLOC_SIZE(&sys_arena, new_cap);
